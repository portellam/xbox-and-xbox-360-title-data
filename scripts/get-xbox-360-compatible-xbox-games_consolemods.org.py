...
def find_table(soup: BeautifulSoup) -> Optional[BeautifulSoup]:
  try:
...
def find_by_header(soup: BeautifulSoup) -> Optional[BeautifulSoup]:
...
def find_by_section_id(soup: BeautifulSoup) -> Optional[BeautifulSoup]:
  ...
  while element:
    ...
    if element and element.name == 'table':
      headers = [th.get_text(strip=True).lower() for th in element.find_all('th')]
...
def process_row(cells: List[BeautifulSoup], headers: List[str]) -> Optional[List[str]]:
  try:
    ...
    for i, cell in enumerate(cells):
      if 0 < i < len(headers) - 2:                                                        # Compatibility columns
        a = cell.find('a')
      ...
      else:                                                                               # Name, Tested By, Known Issues
        row.append(cell.text.strip())
...
def extract_rows(table: BeautifulSoup, headers: List[str]) -> List[List[str]]:
  try:
...
def write_csv(headers: List[str], extracted_rows: List[List[str]], output_file_name: str) -> bool:
  try:
...
def write_json(headers: List[str], extracted_rows: List[List[str]], output_file_name: str) -> bool:
  try:
    output_file = f"{output_file_name}.json"
    print(f"Writing to file: '{output_file}'")
    data = [dict(zip(headers, row)) for row in extracted_rows]
...
def main() -> int:
  ...
  extracted_rows = extract_rows(table, headers)

  if not extracted_rows:
    print("Warning: Could not extract data. No data rows exist.")
    return 1

  if not write_csv(headers, extracted_rows, OUTPUT_FILE_NAME):
    return 1

  if not write_json(headers, extracted_rows, OUTPUT_FILE_NAME):
    return 1
